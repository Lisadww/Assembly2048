...
* 计分机制，用于当前用户按下方向键后计算分数并显示在分数栏
* 计分部分（getscore) 根据当前面板上新出现的方块计算移动后的分数,并显示在分数面板上

### 13.gameEnd

- 检查游戏是否结束，游戏胜利则修改标志win_flag=1;  游戏失败则修改标志lose_flag=1
- 当用户移动方块后，调用gameEnd函数并检查win_flag与lose_flag的值。
  - 在gameEnd模块中分别对 Zero_flag、lose_flag、win_flag赋初值0
  - 遍历方块后，若存在方块数值为2048则修改win_flag的值为1
  - 在win_flag=0（还未合成出2048）的情况下，判断此时矩阵中是否存在为0的方块。若还存在空位，此时lose_flag的值不改变。
  - 当此时不存在空位时，调用IsMerge函数判断此时是否存在可合并方块的情况（判断时需分别对左右、上下两个方向进行判断）
    - IsMerge函数中若不存在可合并情况则修改merge_flag的值为1
  - 当zero_flag=0(不存在空位)且merge_flag=0(不存在可合并情况)时，修改lose_flag的值为一，游戏结果判定为失败。
